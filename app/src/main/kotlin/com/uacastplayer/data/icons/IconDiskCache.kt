...
class IconDiskCache(
    ...
    suspend fun get(url: String): File? = withContext(ioDispatcher) {
     ...
     */
    suspend fun put(url: String, bytes: ByteArray): File? = withContext(ioDispatcher) {
        if (bytes.size > MAX_ICON_BYTES) return@withContext null
        if (ImageFormatDetector.detect(bytes) == null) return@withContext null

        val file = fileFor(url)
        val temp = runCatchingNonFatal { File.createTempFile(file.name, TEMP_SUFFIX, directory) }.getOrNull()
            ?: return@withContext null
        try {
            val published = runCatchingNonFatal {
                temp.writeBytes(bytes)
                publish(temp, file)
            }.onFailure { error ->
                // Worth a line now that a failure here is the difference between a cached icon and
                // none: a full disk, a revoked directory, or a provider-specific filesystem error
                // shows up as icons that silently never stick. The exception's class only - never
                // the url, which is the icon host's, not ours.
                AppLog.w(TAG) { "Icon cache write failed: ${error.javaClass.simpleName}" }
            }.getOrDefault(false)
            if (published) file else null
        } finally {
    ...
    suspend fun trim() = withContext(ioDispatcher) {

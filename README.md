...
Baseline profile: `app/src/main/baseline-prof.txt` is hand-authored (broad wildcard rules for our
own package and the media3/ExoPlayer path), not generated by a Macrobenchmark module - there isn't
one in this project. Coil and Compose already ship their own baseline profiles inside their AARs
and get merged in automatically; media3 doesn't, which is the gap this file fills.
